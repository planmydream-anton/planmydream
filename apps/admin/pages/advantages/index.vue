<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold">Преимущества</h1>
      <UButton to="/advantages/create" icon="i-heroicons-plus">Добавить</UButton>
    </div>
    <UCard>
      <UTable :data="items" :columns="columns">
        <template #isActive-cell="{ row }">
          <UBadge :color="row.original.isActive ? 'success' : 'neutral'" variant="soft">
            {{ row.original.isActive ? 'Активно' : 'Неактивно' }}
          </UBadge>
        </template>
        <template #actions-cell="{ row }">
          <div class="flex gap-1">
            <UButton :to="`/advantages/${row.original.id}`" icon="i-heroicons-pencil" variant="ghost" size="xs" />
            <UButton icon="i-heroicons-trash" variant="ghost" color="error" size="xs" @click="deleteItem(row.original)" />
          </div>
        </template>
      </UTable>
    </UCard>
  </div>
</template>
<script setup lang="ts">
const toast = useToast()
const { data, refresh } = await useFetch<any>('/api/admin/advantages')
const items = computed(() => data.value?.data || [])
const columns = [
  { accessorKey: 'title', header: 'Название' },
  { accessorKey: 'icon', header: 'Иконка' },
  { accessorKey: 'position', header: 'Позиция' },
  { accessorKey: 'isActive', header: 'Статус' },
  { accessorKey: 'actions', header: '' },
]
async function deleteItem(item: any) {
  if (!confirm('Удалить?')) return
  try { await $fetch(`/api/admin/advantages/${item.id}`, { method: 'DELETE' }); toast.add({ title: 'Удалено', color: 'success' }); refresh() }
  catch (e: any) { toast.add({ title: 'Ошибка', description: e.data?.message, color: 'error' }) }
}
</script>
