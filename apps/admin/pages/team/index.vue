<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold">Команда / Организаторы</h1>
      <UButton to="/team/create" icon="i-heroicons-plus">Добавить</UButton>
    </div>
    <UCard>
      <UTable :data="items" :columns="columns">
        ...
        <template #actions-cell="{ row }">
          <div class="flex gap-1">
            <UButton :to="`/team/${row.original.id}`" icon="i-heroicons-pencil" variant="ghost" size="xs" />
...
const { data, refresh } = await useFetch<any>('/api/admin/team')
