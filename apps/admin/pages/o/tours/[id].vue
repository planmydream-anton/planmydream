  ...
  <div class="space-y-6">
    ...
    <div v-show="activeTab === 'basic'" class="space-y-6">
      ...
      <UCard>
        ...
        <div v-if="tourMedia.length" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        ...
        <p v-else class="text-sm text-gray-500">Фотографии не загружены. Рекомендуется до 50 фото.</p>
      </UCard>
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between">
            ...
            <UButton variant="soft" size="xs" icon="i-heroicons-plus" @click="form.keyImpressions.push('')" :disabled="form.keyImpressions.length >= 9" />
          </div>
        </template>
        <div class="space-y-2">
          <div v-for="(_, i) in form.keyImpressions" :key="i" class="flex items-center gap-2">
            <UInput v-model="form.keyImpressions[i]" placeholder="Впечатление..." class="w-full" />
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between">
            ...
            <UButton variant="soft" size="xs" icon="i-heroicons-plus" @click="form.guides.push({ name: '', photo: '', bio: '' })" />
          </div>
        </template>
        <div class="space-y-4">
          <div v-for="(guide, i) in form.guides" :key="i" class="flex items-start gap-3 p-3 border rounded-lg">
            <div class="flex-1 space-y-2">
              <UInput v-model="guide.name" placeholder="Имя гида" class="w-full" />
              <UTextarea v-model="guide.bio" placeholder="Описание" :rows="2" class="w-full" />
            </div>
            <UButton icon="i-heroicons-trash" variant="ghost" color="red" size="sm" @click="form.guides.splice(i, 1)" />
          </div>
          <p v-if="!form.guides.length" class="text-sm text-gray-500">Гиды не добавлены</p>
        </div>
      </UCard>
    </div>
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between">
            ...
            <UButton variant="soft" size="xs" icon="i-heroicons-plus" @click="form.accommodations.push({ name: '', description: '', images: [] })" />
          </div>
        </template>
        <div class="space-y-4">
          <div v-for="(acc, i) in form.accommodations" :key="i" class="p-4 border rounded-lg space-y-3">
            <div class="flex items-center justify-between">
              <UInput v-model="acc.name" placeholder="Название отеля / места" class="w-full mr-3" />
    ...
    <div v-show="activeTab === 'prices'" class="space-y-6">
      ...
      <!-- Departures will be managed separately -->
      <UCard>
    ...
    <div v-show="activeTab === 'terms'" class="space-y-6">
        ...
        <div class="space-y-4">
          <div>
            <div class="flex items-center justify-between mb-2">
              ...
              <UButton variant="soft" size="xs" icon="i-heroicons-plus" @click="form.includes.push({ category: 'other', text: '' })" />
            </div>
            <div class="space-y-2">
              <div v-for="(item, i) in form.includes" :key="i" class="flex items-center gap-2">
                <USelect v-model="item.category" :items="includeCategoryOptions" class="w-40" />
                <UInput v-model="item.text" placeholder="Описание" class="w-full" />
          ...
          <div>
            ...
            <div class="space-y-2">
              <div v-for="(_, i) in form.excludes" :key="i" class="flex items-center gap-2">
                <UInput v-model="form.excludes[i]" placeholder="Описание" class="w-full" />
      ...
      <UCard>
        ...
        <UTextarea v-model="form.insurance" :rows="3" class="w-full" />
      </UCard>
    ...
    <!-- Tab: Города -->
    <div v-show="activeTab === 'cities'" class="space-y-6">
      <UCard>
      ...
      <UCard>
        <template #header><h3 class="font-semibold">Комментарий для туристов</h3></template>
        <UTextarea v-model="form.generalTouristComment" :rows="3" class="w-full" />
      </UCard>

      <UCard>
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="font-semibold">Города прибытия / отправления</h3>
            <UButton variant="soft" size="xs" icon="i-heroicons-plus" @click="form.cities.push({ city: '', canArrive: true, canDepart: true, comment: '' })" />
          </div>
        </template>
        <div class="space-y-3">
          <div v-for="(city, i) in form.cities" :key="i" class="flex items-start gap-3 p-3 border rounded-lg">
            <div class="flex-1 space-y-2">
              <UInput v-model="city.city" placeholder="Название города" class="w-full" />
              <div class="flex items-center gap-4">
                <label class="flex items-center gap-2 text-sm">
                  <UCheckbox v-model="city.canArrive" /> Прибытие
                </label>
                <label class="flex items-center gap-2 text-sm">
                  <UCheckbox v-model="city.canDepart" /> Отправление
                </label>
              </div>
              <UInput v-model="city.comment" placeholder="Комментарий" class="w-full" />
            </div>
            <UButton icon="i-heroicons-trash" variant="ghost" color="red" size="sm" @click="form.cities.splice(i, 1)" />
          </div>
          <p v-if="!form.cities.length" class="text-sm text-gray-500">Города не добавлены</p>
        </div>
      </UCard>
    </div>
  </div>
...
const form = reactive({
  ...
  keyImpressions: [] as string[],
  guides: [] as Array<{ name: string; photo?: string; bio?: string }>,
  accommodations: [] as Array<{ name: string; description: string; images: string[] }>,
  program: [] as Array<{ day: number; title: string; content: string }>,
  includes: [] as Array<{ category: string; text: string }>,
  excludes: [] as string[],
  cities: [] as Array<{ city: string; canArrive: boolean; canDepart: boolean; comment?: string }>,
})
...
function getFormBody() {
  return {
    ...
    keyImpressions: form.keyImpressions.filter(Boolean),
    ...
    includes: form.includes.filter(i => i.text),
